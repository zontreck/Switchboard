            ...
            case "PUT": {
                if($username) {
                    ...
                    foreach ($DEFAULT_USER_FIELDS as $value) {
                        ...
                        $dfStmt->bind_param("sssii", $userid, $fieldID, $value["Name"], $value["Type"], $value["Order"]);
                        $dfStmt->execute();
                        $dfStmt->close();
                        $DB->commit();
                    }
                    ...
                    $rfTime = time();
                    $rootName = "root";
                    $rfStmt = $DB->prepare("INSERT INTO `Folders` (`ID`, `Name`, `ParentFolder`, `UserID`, `Created`, `Modified`) VALUES (?, ?, ?, ?, ?, ?);");
                    $rfStmt->bind_param("ssssii", $rootFolder, $rootName, $null, $userid, $rfTime, $rfTime);
                    $rfStmt->execute();
                    $rfStmt->close();
                    $DB->commit();
    ...
    case preg_match('#^/alters(?:/([^/]+))?$#', $route, $matches) === 1: {
        ...
        switch($request) {
            case "GET": {
                ...
                $rres=null;
                if($AlterCount == -1) {
                    $stmt = $DB->prepare("SELECT * FROM Alters WHERE User='$userid' ORDER BY SubID ASC;");
                    $stmt->bind_param("s", $userid);
                    $stmt->execute();
                    $rres = $stmt->get_result();
                } else {
                    $stmt = $DB->prepare("SELECT * FROM Alters WHERE User=? ORDER BY SubID ASC LIMIT ? OFFSET ? ;");
        ...
        if($success) {
            switch($request) {
                ...
                case "PUT": {
                    ...
                    $proxies = json_encode($alter['proxies']);
                    $proxyName = $alter['proxy_name'] ?? "";

                    $stmt = $DB->prepare("INSERT INTO Alters (User, ID, Name, Avatar, Fields, SubID, ParentID, Flags, ProxyName, Proxies) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);");
                    $stmt->bind_param("ssssbisiss", $SBAuth->UserID, $alterId, $alter['name'], $alter['avatar'], $null, $alter['subid'], $alter['parent'], $nFlags, $proxyName, $proxies);
